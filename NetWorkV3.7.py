...
def back_prop(input_size, HL, nHL, W_in, H_in, W_hid, H_hid, W_out, H_out, in_data_set, error, out_size, upd_W_in,
              upd_W_hid, upd_W_out):
    error_new = []
    if (out_size == 1):
        for l in range(nHL):
    ...
    for L in range ((HL-2),-2,-1):
        ...
        for l in range(nHL):
            for r in range(nHL):
                ...
                upd_W_out[(L+1),l,r]=error[r,0]*H_hid[l,L]*H_hid[r,(L+1)]*(1.0-(H_hid[r,(L+1)]))
