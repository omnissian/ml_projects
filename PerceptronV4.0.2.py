...
#C:\\Users\\Anatoly\\Desktop\\Work1\\plgn\\data10by10digitstest\\forlearn7\\
# data6by6digitstest
def parser(labels, path='C:\\Users\\Anatoly\\Desktop\\Work1\\plgn\\data10by10digitstest\\forlearn7\\'):
# def parser(labels, path='C:\\Users\\Anatoly\\Desktop\\Work1\\plgn\\data2by2digits\\forlearn7\\'):
    data_set = []
...
def activ_func(scalar_value):
    ...
    # tmp= scalar_value * (-1) #v1
    # # np.exp(np.array([1391.12694245], dtype=np.float128))
    #
    # # scalar_value = ((1.0) / (1.0 + np.exp(np.array([tmp], dtype=np.float128))))
    # # ----
    # scalar_value = ((1.0) / (1.0 + np.exp(np.array([tmp], dtype=np.longdouble)))) #v1

    # return ((1.0)/(1.0+0.9999*(2.71828**(-scalar_value))))
    out_val=((1.0)/(1.0+(2.71828**(-scalar_value))))
    # return 0.999*((1.0)/(1.0+(2.71828**(-scalar_value))))
    return ((1.0)/(1.0+(2.71828**(-scalar_value))))
...
def back_prop(input_size, HL, nHL, W_in, H_in, W_hid, H_hid, W_out, H_out, in_data_set, error, out_size, upd_W_in, upd_W_hid, upd_W_out, learning_rate=0.001):
    # error_new = []
    current_error=error
    error_new=np.zeros((nHL,out_size),dtype=np.float64)
    if (out_size == 1):
        for l in range(nHL):
            for r in range(out_size):
                ...
                # if(not((1.0-H_out[r,0]))):
                #     upd_W_out[l, r] = error * H_hid[l, (HL - 1)] * H_out[r, 0] * 0.99999*(1.0 - H_out[r, 0])
                # else:
                #     upd_W_out[l, r] = error * H_hid[l, (HL - 1)] * H_out[r, 0] * (1.0 - H_out[r, 0])

                tmp_val = H_out[r, 0] * (1.0 -H_out[r, 0])#0.9999*
                error_new[l, 0] = W_out[l, 0] * error * tmp_val
                upd_W_out[l,r]=error*tmp_val*H_hid[l,(HL-1)] # because of derivative of sigmoid new
    ...
    for L in range ((HL-2),-1,-1):
        ...
        for l in range(nHL):
            ...
            for r in range(nHL):
                ...
                # if (not (1.0-(H_hid[r,(L+1)]))):
                #     upd_W_hid[(L+1),l,r]=error[r,0]*H_hid[l,L]*H_hid[r,(L+1)]*(1.0-0.99999*(H_hid[r,(L+1)]))
                # else:
                #     upd_W_hid[(L + 1), l, r] = error[r, 0] * H_hid[l, L] * H_hid[r, (L + 1)] * (1.0 - (H_hid[r, (L + 1)]))
                tmp_val=H_hid[r,(L+1)]*(1.0-(H_hid[r,(L+1)]))#0.9999*
                error_new[l, 0] = tmp_err[0, 0]*tmp_val
                upd_W_hid[(L+1),l,r]=error[r,0]*tmp_val*H_hid[l,L]
                # upd_W_hid[(L+1),l,r]=error[r,0]*H_hid[l,L]*H_hid[r,(L+1)]*(1.0-(H_hid[r,(L+1)]))
    ...
    for l in range(nHL):
        ...
        for r in range(nHL):
            ...
            #------------------------------------------------------------------
            # if (not ((1.0-(H_hid[r,0])))):
            #     upd_W_hid[0, l, r] = error[r, 0] * H_in[l, 0] * H_hid[r, 0] * (1.0 - 0.99999*(H_hid[r, 0]))
            # else:
            #     upd_W_hid[0, l, r] = error[r, 0] * H_in[l, 0] * H_hid[r, 0] * (1.0 - (H_hid[r, 0]))
            tmp_val=H_hid[r,0]*(1.0-(H_hid[r,0]))##0.9999*
            error_new[l, 0] = tmp_err[0, 0] *tmp_val
            upd_W_hid[0,l,r]=error[r,0]*H_in[l,0]*tmp_val
            # upd_W_hid[0,l,r]=error[r,0]*H_in[l,0]*H_hid[r,0]*(1.0-(H_hid[r,0]))
    error=np.copy(error_new) #error=error_new.copy()
    ...
    for l in range(input_size):
        for r in range(nHL):
            ...
            #-----------------------------------------------------
            # if(not(1.0-(H_in[r,0]))):
            #     upd_W_in[l,r]=error[r,0]*i[l,0]*H_in[r,0]*(1.0-0.99999*(H_in[r,0]))
            # else:
            #     upd_W_in[l, r] = error[r, 0] * i[l, 0] * H_in[r, 0] * (1.0 - (H_in[r, 0]))
            upd_W_in[l,r]=error[r,0]*i[l,0]*H_in[r,0]*(1.0-(H_in[r,0])) #0.9999*
            # upd_W_in[l,r]=error[r,0]*i[l,0]*H_in[r,0]*(1.0-(H_in[r,0]))
...
W_in=upd_W_hid=np.random.standard_normal(size=(input_size,nHL))
H_in = np.zeros((nHL, 1),dtype=np.float64)  # vector E R^(nHLx1)
# ----------------hidden layer----------------
# W_hid = np.random.rand(HL, nHL, nHL)  # weights
W_hid=np.random.standard_normal(size=(HL,nHL,nHL))
H_hid = np.zeros((nHL, HL), dtype=np.float64)  # matrix ofr output values for each neuron in hidden layers layer
# ----------output layer----------
# W_out = np.random.rand(nHL, out_size)  # weights
W_out=np.random.standard_normal(size=(nHL,out_size))
H_out = np.zeros((out_size, 1),dtype=np.float64)
# -------------------------------------------
learning_rate = 0.01  # just a constant value for a while, maybe add a "momentum" in the fьюча or would make a custom "changer" for it

iterations = 10 ** 9  # number of iterations
c=5
#----learn------------#
for iteration in range(iterations):
    ...
    if (not(iteration%c)):
        print("-------------------------------------")
        print("please in '0' for stop learning, or")
        print("number of epochs for continue learning")
        c=int(input())
        print("-------------------------------------")
        if(c==0):
            break

        # print(iteration, " iterrations passed---------------")
    else:
        pass
    for exmpl in range(count_of_examples):
        # print("number of example= ", exmpl)
        upd_W_in=np.zeros((input_size,nHL),dtype=np.float64)
        upd_W_hid=np.zeros((HL,nHL,nHL),dtype=np.float64)
        upd_W_out=np.zeros((nHL,1),dtype=np.float64)
        checker=np.zeros((nHL,1),dtype=np.float64)
        # upd_W_in=np.random.standard_normal(size=(input_size,nHL))
        # upd_W_hid=np.random.standard_normal(size=(HL,nHL,nHL))
        # upd_W_out=np.random.standard_normal(size=(nHL,1))
        # checker=np.random.standard_normal(size=(nHL,1))
        # s = np.random.standard_normal(size=(3, 4, 2))
        #---------------learning rate-----------------
        # random.uniform(0.00001, 0.03)
        learning_rate=np.random.uniform(0.00001, 0.08)
        # ---------------learning rate-----------------
        ...
        tmp_er_prnt=0.5*(error**2)
        error_print=error_print+tmp_er_prnt
    error_print=error_print/count_of_examples
    # print("delta errors = ",error_old-error_print)
    # error_old=error_print
    print("after epoch №", iteration)
    print("error squared = ", error_print)
    print("W hid 0 0 = ", W_hid[0,0,0])
    print("upd W hid 0 0 = ", upd_W_hid[0,0,0])

print("da")

#check
labels_examine=[]
print("do you want to choose tha path for yours examine examples of data?")
print("Y/N?")
c=input()
if(c=="Y" or c=="y" ):
    path_examine=input()
else:
    path_examine='C:\\Users\\Anatoly\\Desktop\\Work1\\plgn\\data10by10digitstest\\forcheck7\\'
# cycler=1
test_iter=0
while (test_iter<3):
    print("-----------------------------------------------------------------")
    print("now you may change the testing data in the next path:")
    print(path_examine)
    print("do you want to continue testing after this test?")
    print("Y/N?")
    answer=input()
    if (answer=="Y" or answer=="y"):
        ...
        test_iter=3
    # examine_set=parser(labels,path_examine)
    labels_examine=[]
    examine_set = np.array(parser(labels_examine,path_examine), dtype=float)
    count_of_examples, exmpl_vector_d_size = examine_set.shape
    # input_size = exmpl_vector_d_size
    for exmpl in range(count_of_examples):
        print("---------------------------------------------------------")
        outval = forward(exmpl_vector_d_size, HL, nHL, W_in, H_in, W_hid, H_hid, W_out, H_out, examine_set[exmpl], labels_examine[exmpl], out_size,0)
        print ("out val=", outval)
        print ("label= ", labels_examine[exmpl])

# examine_set=parser(labels_examine,)
