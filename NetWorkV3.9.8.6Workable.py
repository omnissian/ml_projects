...
def activ_func(scalar_value):
    ...
    scalar_value = ((1.0) / (1.0 + np.exp(np.array([tmp], dtype=np.longdouble))))
...
# -------------------------------------------
learning_rate = 0.1  # just a constant value for a while, maybe add a "momentum" in the fьюча or would make a custom "changer" for it
