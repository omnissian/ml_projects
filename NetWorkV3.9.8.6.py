...
def back_prop(input_size, HL, nHL, W_in, H_in, W_hid, H_hid, W_out, H_out, in_data_set, error, out_size, upd_W_in, upd_W_hid, upd_W_out, learning_rate=0.001):
    ...
    if (out_size == 1):
        for l in range(nHL):
            for r in range(out_size):
                ...
                tmp_val = H_out[r, 0] * (1.0 - 0.9999*H_out[r, 0])
    ...
    for L in range ((HL-2),-1,-1):
        ...
        for l in range(nHL):
            ...
            for r in range(nHL):
                ...
                # else:
                #     upd_W_hid[(L + 1), l, r] = error[r, 0] * H_hid[l, L] * H_hid[r, (L + 1)] * (1.0 - (H_hid[r, (L + 1)]))
                tmp_val=H_hid[r,(L+1)]*(1.0-0.9999*(H_hid[r,(L+1)]))
    ...
    for l in range(nHL):
        ...
        for r in range(nHL):
            ...
            tmp_val=H_hid[r,0]*(1.0-0.9999*(H_hid[r,0]))
            error_new[l, 0] = tmp_err[0, 0] *tmp_val
    ...
    for l in range(input_size):
        for r in range(nHL):
            ...
            upd_W_in[l,r]=error[r,0]*i[l,0]*H_in[r,0]*(1.0-0.9999*(H_in[r,0]))
            # upd_W_in[l,r]=error[r,0]*i[l,0]*H_in[r,0]*(1.0-(H_in[r,0]))
...
# -------------------------------------------
learning_rate = 10  # just a constant value for a while, maybe add a "momentum" in the fьюча or would make a custom "changer" for it
